function [] = Collect_GOESR(istorm, DAtimes_str, control)

    % Determine if a subdirectory for a storm object exists
    if ~exist([control.obs_collect_dir],'dir')
        [~, msg, ~] = mkdir(control.obs_collect_dir);% create a subdirectory for a storm object under Collected_MW/
        if isempty(msg)
            disp(['  Successfully created a subdirectory in ',control.obs_collect_dir]); 
        else
            error('  Error: ',msg);
        end
        destination = [control.obs_collect_dir];
    else
    ...
    for ich = 1:length(control.favCH) 
        ...
        for i = 1:length(Tb_files)
            ...
            if use_Tb_file == 0
                ...
                ir = find(time_start == DAtimes_str);
                if isempty(ir) 
                    continue;
                else
                    [filepath,filename,filext] = fileparts(Tb_file);
                    source_file = erase(Tb_file,'Preprocess_Obs/raw_Obs/Collected_IR/');
		    newfile_name = ['DAt' + DAtimes_str(ir) + '_GOESR_ABI_L2_CMIPF_CH' + num2str(control.favCH(ich)) + '.nc'];
