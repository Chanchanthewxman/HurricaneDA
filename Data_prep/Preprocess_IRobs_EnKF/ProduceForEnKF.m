function [myTimes,mySat_name,myChNum,myLat,myLon,myTb,myROI_hydro,myROI_other,myObsErr,mySat_alt] = ProduceForEnKF(istorm, DAtime, Ch, Tb_file, control)
    ...
    % -------------------------------------------------------------------------
    % Read domain of interest from geo_em.d0*.nc generated by WPS geogrid.exe
    % ------------------------------------------------------------------------
    if control.fix_domain
        geo_file =  [control.geogrid_dir,'/geo_em.',control.domain,'.nc'];
    else
        geo_file =  [control.geogrid_dir,'/',DAtime,'/geo_em.',control.domain,'.nc'];
    end
	...
	if numel(control.filter_reso) == 1
		grid_start = floor(filter_grid_step(1) / 2); % start point  
	elseif numel(control.filter_reso) == 2
		...
		disp('Current data-thinning method does NOT support more than two plans!'); 
	end
    ...
    for iroi = 1:length(control.roi_oh)
		...
		[ idx_PickObs] = Knnsearch_PickRawforCRTM(lat_obs_toPick,lon_obs_toPick,xlat_m,xlon_m,grid_start(iroi),filter_grid_step(iroi));
	...
	if numel(control.filter_reso) == 2
...
end % End function ProduceForEnKF() 

% -------------------------------------------------------------------------
% Fancy function from ChatGPT: Find Nearest GOES Pixel for Each WRF Point
% -------------------------------------------------------------------------
function [ idx_getObs ] = Knnsearch_PickRawforCRTM( obs_lat,obs_lon,m_xlat,m_xlon,start_grid,step_grid )

    % Filter lat and lon of WRF domain for each ROI
    idx_step_WestEast = start_grid:step_grid:size(m_xlon,1);
    idx_step_SouthNorth = start_grid:step_grid:size(m_xlon,2);

    xlat = m_xlat(idx_step_WestEast, idx_step_SouthNorth);
    xlon = m_xlon(idx_step_WestEast, idx_step_SouthNorth);
    xlat_col = xlat(:);
    xlon_col = xlon(:);

    % Convert lat/lon to 3D Cartesian coordinates on unit sphere
    latlon2xyz = @(lat,lon) [cosd(lat).*cosd(lon), cosd(lat).*sind(lon), sind(lat)];

    goes_xyz = latlon2xyz(obs_lat,obs_lon);
    wrf_xyz = latlon2xyz(xlat_col,xlon_col);

    % Find nearest neighbor
    % knnsearch(X,Y) finds the nearest neighbor in X for each query point in Y and returns the indices of the nearest neighbors in Idx, a column vector. Idx has the same number of rows as Y.
    idx_getObs = knnsearch(goes_xyz, wrf_xyz);
