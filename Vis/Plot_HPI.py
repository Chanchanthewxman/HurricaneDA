...
def read_bestrack( Storm ):
    ...
    # Identify the file
    btk_dir = small_dir + Storm + '/TC_Guidance/'
...
def plot_hpi_df( wrf_dir, Storm, Exper, domain_range ):
    ...
    for iExper in Exper:
        if iExper is not None:
            if os.path.exists( wrf_dir+'/'+Storm+'/'+iExper+'/' ):
                Exper_content_lbl[iExper] = sorted(fnmatch.filter(os.listdir( wrf_dir+'/'+Storm+'/'+iExper+'/' ),'20*')) 
                # manually set 
    ...
    for key in Exper_content_lbl: 
        ...
        if Exper_content_lbl[key] is not None:
            ...
            for it in Exper_content_lbl[key]:
                print(wrf_dir+'/'+Storm+'/'+key+'/'+it)
                HPI_model = read_rsl_error(Storm, key, wrf_dir+'/'+Storm+'/'+key+'/'+it, it, DF_model_end)
    ...
    # Save the figure
    des_name = Storm+'_forecast_test.png'
    plt.savefig( des_name )
    print( 'Saving the figure to '+des_name+'!' )

if __name__ == '__main__':
    
    big_dir = '/expanse/lustre/scratch/zuy121/temp_project/Pro2_PSU_MW/' # where forecasts (input data) are 
    small_dir = '/expanse/lustre/projects/pen116/zuy121/Pro6_Chandler/'

    # configuration
    Storm = 'IRMA'
    Exper_name = ['CONV_WSM6',]
